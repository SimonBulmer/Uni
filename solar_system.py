class Moon:
    '''A class to store the name of moon.'''
...
class Planet:
    ...
    def create_planets():
        # Information taken from wikipedia - https://en.wikipedia.org/wiki/List_of_gravitationally_rounded_objects_of_the_Solar_System
...
class Queries:
    ...
    def display_all_planet_info(self):
        print(f"=" * 90)
            ...
            if name in query:
                ...
                if any (word in query for word in["everything","info","details","notes"]):
                    ...
                    if moon_count < 1:
                        ...
                        print(f"{planet.name} has {moon_count} {no_of_moons}: Some of the popular moons are {', '.join(str(m) for m in moons) if moons else 'None'}.")
                    return
                else:
                    print(f"What do you want to know about {planet.name}?")
                    return
